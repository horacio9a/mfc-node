// MyFreeCams File Converter for ffmpeg v.2.8.4
  ...
  return new Promise((resolve, reject) => {
    ...
    ffprobeProcess.on('close', code => {
      if (code !== 0) {
        ...
        fs.renameAsync(srcFile, `${srcFile}.no-audio`);
...
function getSpawnArguments(srcFile, dstFile) {
  return getAudioCodec(srcFile)
    .then(audioCodec => (audioCodec === 'aac')
      ? ['-i', srcFile,'-y','-hide_banner','-loglevel','panic','-c:v','copy','-c:a','copy','-bsf:a','aac_adtstoasc','-copyts','-start_at_zero',dstFile] // aac
      : ['-i', srcFile,'-y','-hide_banner','-loglevel','panic','-c:v','copy','-c:a','libvo_aacenc','-b:a','128k',dstFile])}; // speex or something else
...
function convertFile(srcFile) {
  ...
  return mkdirpAsync(dstPath)
    ...
    .then(spawnArguments => new Promise((resolve, reject) => {
      ...
      // printMsg('ffmpeg284 ' + spawnArguments.join(' '));

      let ffmpegProcess = childProcess.spawn('ffmpeg284', spawnArguments);
          ...
          fs.statAsync(src)
            .then(stats => {
              ...
              if (!config.deleteAfter) {
                return fs.renameAsync(src, `${src}.bak`);
              }

              return fs.statAsync(tempDst).then(tempStats => {
                if ((Math.abs(stats.size - tempStats.size) / tempStats.size) < 0.1) {
                  return fs.unlinkAsync(src);
                } else {
                  return fs.renameAsync(src, `${src}.bak`);
                }
              });
