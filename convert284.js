...
function getAudioCodec(srcFile) {
  return new Promise((resolve, reject) => {
    ...
    let ffprobeProcess = childProcess.spawn('ffprobe284', spawnArguments);
...
function getSpawnArguments(srcFile, dstFile) {
  return getAudioCodec(srcFile)
    .then(audioCodec => (audioCodec === 'aac')
      ? ['-i', srcFile,'-y','-hide_banner','-loglevel','panic','-movflags','+faststart','-c:v','copy','-c:a','libvo_aacenc','-b:a','128k','-copyts','-start_at_zero',dstFile]
