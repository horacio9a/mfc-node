...
var dstDirectory = path.resolve(config.dstDirectory || 'C:/Videos/MFC/Converted');
...
function getFiles() {let files = [];

  return new Promise((resolve, reject) => {filewalker(srcDirectory, { maxPending: 1, matchRegExp: /(\.ts|\.flv)$/ })
      .on('file', (p, stats) => {
...
function getAudioCodec(srcFile) {
  return new Promise((resolve, reject) => {
    ...
    ffprobeProcess.on('close', code => {
      if (code !== 0) {reject(`Failed to get audio codec from ${srcFile}`);
      } else {resolve(audioCodec)}})}).timeout(5000); // 5 seconds}

function getSpawnArguments(srcFile, dstFile) {return getAudioCodec(srcFile).then(audioCodec => (audioCodec === 'aac')
      ? [ // aac'-i', srcFile,'-y','-hide_banner','-loglevel', 'panic','-movflags', '+faststart','-c:v','copy','-c:a','copy','-bsf:a','aac_adtstoasc','-copyts','-start_at_zero',dstFile]
...
function mainLoop() {
  ...
  Promise
    .try(() => getFiles())
    .then(files => new Promise((resolve, reject) => {printMsg(files.length + ' file(s) to convert');

      if (files.length === 0) {resolve();
      } else {files.forEach(file => {queue
            .add(() => convertFile(file))
            .then(() => {if ((queue.getPendingLength() + queue.getQueueLength()) === 0) {resolve()}})
            .catch(err => {printErrorMsg(err)})})}}))
    .catch(err => {if (err) {printErrorMsg(err)}})
    .finally(() => {let seconds = startTs - moment().unix() + dirScanInterval;
