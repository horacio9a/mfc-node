// MyFreeCams HD Recorder v.3.0.1
...
var config = yaml.safeLoad(fs.readFileSync('config-hd.yml', 'utf8'));
...
function getOnlineModels(proxyModels) {
  ...
  if (proxyModels.length > 0) {
    ...
    printDebugMsg(`${newModels.length} new HD model(s) from proxy ${colors.green(config.proxyServer)}`);

    // merge newModels with "local" models
    onlineModels = newModels.concat(models)} else {onlineModels = models}

  printMsg(`${onlineModels.length} HD model(s) online.`)}

// goes through the models in the queue and updates their settings in config
function updateConfigModels() {printDebugMsg(`${config.queue.length} HD model(s) in the queue.`);
...
function selectModelsToCapture() {printDebugMsg(`${config.models.length} HD models in 'config-hd.yml'`);
  ...
  printDebugMsg(`${modelsToCapture.length} HD model(s) to recording.`);
...
let dlProgram;
   ...
   if (config.downloadProgram == 'rtmp') {dlProgram = 'rtmpdump'}
...
function createMainCaptureProcess(model) {
  return Promise
    ...
    .then(packet => {
      ...
      let hlsUrl = `https://video${server}.myfreecams.com:8444/x-hls/${cxid}/${roomId}/${pwd}/${ctx}/mfc_${ph}_${roomId}.m3u8`;

      let captureProcess;
         if (config.downloadProgram == 'ls') {captureProcess = childProcess.spawn(dlProgram, ['-Q','hlsvariant://' + hlsUrl,'best','-o',path.join(captureDirectory, filename)])};
         if (config.downloadProgram == 'sl') {captureProcess = childProcess.spawn(dlProgram, ['-Q','hls://' + hlsUrl,'best','-o',path.join(captureDirectory, filename)])};
         if (config.downloadProgram == 'ff-ts') {captureProcess = childProcess.spawn(dlProgram, ['-hide_banner','-v','fatal','-i',hlsUrl,'-c','copy','-vsync','2','-r','60','-b:v','500k',path.join(captureDirectory, filename)])};
         if (config.downloadProgram == 'ff-flv') {captureProcess = childProcess.spawn(dlProgram, ['-hide_banner','-v','fatal','-i',hlsUrl,'-c:v','copy','-c:a','aac','-b:a','192k','-ar','32000',path.join(captureDirectory, filename)])};
         if (config.downloadProgram == 'hls') {captureProcess = childProcess.spawn(dlProgram, [hlsUrl,'-q','-o',path.join(captureDirectory, filename)])};
         if (config.downloadProgram == 'rtmp') {captureProcess = childProcess.spawn(dlProgram, ['-q','-r',`rtmp://video${server}.myfreecams.com/NxServer`,'-a','NxServer','-CN:${sid}','-CS:${pwd}','-CS:${roomId}',
             '-CS:${ph}','-CS:DOWNLOAD','-CN:${model.uid}','-CN:0','-CS:${ctx}','-y',`mfc_${ph}_${roomId}?ctx=${ctx}&tkx=${pwd}`,config.rtmpDebug ? '-V' : '','-m',60,'-o',path.join(captureDirectory, filename)])};
...
function createCaptureProcess(model) {if (model.camserv < 840) {return}; // skip models without "mobile feed"
  ...
  if (captureModel !== undefined) {printMsg(colors.yellow(`>>> ` + captureModel.filename + ` ${colors.gray(`* HD * ` + dlProgram + ` <<<`)}`));

    return};

  printMsg(colors.green(model.nm) + ` now online - starting HD ${colors.yellow(dlProgram)} recording process.`);
...
function saveConfig() {if (!isDirty) {return};
  ...
  printDebugMsg(`Save changes in 'config-hd.yml'`);

  return fs
    .writeFileAsync('config-hd.yml', yaml.safeDump(config).replace(/\n/g, EOL), 'utf8')
    .then(() => {isDirty = false})};
...
function mainLoop() {printDebugMsg(`Start new cycle.`);
  ...
  Promise
    ...
    .then(cacheModels)
    .catch(printErrorMsg)
    .finally(() => {printMsg(`Done - will search for new HD models in ${config.modelScanInterval} seconds.`);
...
dispatcher.onGet('/', (req, res) => {fs.readFile(path.join(__dirname, 'index-hd.html'), (err, data) => {
